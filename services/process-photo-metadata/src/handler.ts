...
function extractTakenAtFromFilename(filename: string): Date | null {
  ...
  const isoWithTime = filename.match(/(\d{4})-(0[1-9]|1[0-2])-(0[1-9]|[12]\d|3[01])[\s_\-](\d{2})[.:\-](\d{2})[.:\-](\d{2})/);
  if (isoWithTime) {
  ...
  const withTime = filename.match(/(\d{4})(0[1-9]|1[0-2])(0[1-9]|[12]\d|3[01])[_\-](\d{2})(\d{2})(\d{2})/);
  if (withTime) {
  ...
  const dateOnly = filename.match(/(\d{4})(0[1-9]|1[0-2])(0[1-9]|[12]\d|3[01])/);
  if (dateOnly) {
...
function extractTakenAt(exifBuffer: Buffer | undefined, filename: string): Date | null {
  if (exifBuffer) {
    try {
  ...
  for (const sqsRecord of event.Records) {
    try {
      ...
      await db.insert(photos)
        ...
        .onConflictDoUpdate({ target: photos.s3Key, set: { status: 'processing' } });
